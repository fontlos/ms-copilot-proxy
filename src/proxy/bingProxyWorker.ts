...
/** websocket */
async function websocketProxy(request: Request, hostname: string): Promise<Response> {
...
const bingProxyLink = newProxyLinkHttp<Env>({
	async intercept(req, env) {
		...
		if (upgradeHeader && upgradeHeader == 'websocket') {
			if (reqUrl.pathname.startsWith('/sydney/')) {
				return websocketProxy(req, 'sydney.bing.com');
			}
			if (reqUrl.pathname.startsWith('/c/api/')) {
				return websocketProxy(req, 'copilot.microsoft.com');
			}
		}
		// 处理 CORS 请求
	...
	async reqTranslator(config, req, env) {
		//修改请求
		{
			//基础转换
			config.url.port = '';
...

		{
			...
			// login account请求
			if (p.startsWith('/proofs/') || p == '/SummaryPage.aspx') {
...

		{
			...
			if (p.toLocaleLowerCase().startsWith('/identity/') || p == '/proofs/Add') {
				...
				if (requrl) {
					url.searchParams.set('ru', requrl.replace(proxyOrigin, 'https://copilot.microsoft.com'));
				}
			}
			if (p == '/login.srf') {
				let requrl = url.searchParams.get('wreply');
				if (requrl) {
					url.searchParams.set('wreply', requrl.replace(proxyOrigin, 'https://copilot.microsoft.com'));
				}
			}
			// 修改更新会话请求
	...
	async resTranslator(config, res, req, env) {
		//修改返回
		const reqUrl = new URL(req.url);
		//基本转换
		config.init.headers = new Headers(config.init.headers);
...

		{
			...
			if (
				resUrl.pathname != '/turing/captcha/challenge' && // 这个路径是验证接口，不进行处理
				contentType &&
...
export const bingProxyWorker = (req: Request, env: Env) => {
	// 初始化 环境变量
